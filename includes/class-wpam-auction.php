...
class WPAM_Auction
{
	...
	public function __construct()
	{
		...
		add_filter('woocommerce_product_data_tabs', array(
			$this,
			'reorder_product_tabs'
		) , 20);
		add_action('woocommerce_product_data_panels', array(
			$this,
			'add_product_data_fields'
		));
		add_action('woocommerce_process_product_meta_auction', array(
			$this,
			'save_product_data'
		));
		add_action('init', array(
		...
		add_action('wpam_check_ended_auctions', array(
			$this,
			'check_ended_auctions'
		));
		add_action('wpam_update_auction_states', array(
			$this,
			'update_auction_states'
		));
		add_action('wpam_auction_start', array(
			$this,
			'handle_auction_start'
		));
		add_action('wpam_auction_end', array(
			$this,
			'handle_auction_end'
		));
		add_action('wpam_auction_suspend', array(
			$this,
			'handle_auction_suspend'
		));
		add_action('wpam_auction_resume', array(
			$this,
			'handle_auction_resume'
		));
		add_action('wpam_auction_cancel', array(
			$this,
			'handle_auction_cancel'
		));
		add_action('wpam_send_auction_reminders', array(
			$this,
		...
		{
			\WP_CLI::add_command('wpam auction-status', array(
				$this,
				'cli_auction_status'
			));
		}
	...
	{
		register_post_meta('product', '_auction_state', array(
			'show_in_rest' => true,
			'single' => true,
			'type' => 'string',
		...
		register_post_meta('product', '_auction_status', array(
			'show_in_rest' => true,
			'single' => true,
			'type' => 'string',
			'auth_callback' => function ($allowed, $meta_key, $post_id)
		...
		register_post_meta('product', '_auction_ending_reason', array(
			'show_in_rest' => true,
			'single' => true,
			'type' => 'string',
			'auth_callback' => function ($allowed, $meta_key, $post_id)
	...
	public function register_product_type()
	{
		...
		add_filter('woocommerce_product_class', array(
			$this,
			'register_auction_class'
		) , 10, 2);
	}
	...
	public function add_product_data_fields()
	{
		...
		woocommerce_wp_text_input(array(
			...
			'desc_tip' => true,
			'value' => $start_value,
			'custom_attributes' => array_merge($is_past_start ? array(
				'disabled' => 'disabled'
			) : array() , array(
				'data-help' => $desc
			)) ,
		...
		echo '<p>' . sprintf(esc_html__('Current time: %1$s (%2$s, UTC%3$s)', 'wpam') , esc_html(wp_date('Y-m-d H:i:s', $timestamp, $timezone)) , esc_html($timezone->getName()) , esc_html(sprintf('%+d', $offset_hours))) . '</p>';
		echo '</div>';
	...
	public function save_product_data($post_id)
	{
		...
		if (isset($_POST['_auction_end']))
		{
			...
			try
			{
				...
				{
					wp_clear_scheduled_hook('wpam_auction_end', array(
						$post_id
					));
					wp_schedule_single_event($timestamp, 'wpam_auction_end', array(
						$post_id
					));
				}
	...
	public function handle_auction_start($auction_id)
	{
		...
		WPAM_Event_Bus::dispatch('auction_status', array(
	...
	public function handle_auction_end($auction_id)
	{
		...
		if (get_post_meta($auction_id, '_auction_order_id', true))
		{
			return;
		}
		...
		try {
			...
			$existing_order_id = get_post_meta( $auction_id, '_auction_order_id', true );
			...
			if ( $existing_order_id ) {
				update_post_meta( $auction_id, '_auction_order_id', $existing_order_id );
				$wpdb->query( 'COMMIT' );
				return;
			}
			...
			$order->update_meta_data( 'wpam_auction_id', $auction_id );
			$order->save();
	...
	private function relist_auction($auction_id)
	{
		...
		{
			as_schedule_single_action($start_ts, 'wpam_auction_start', array(
				$auction_id
			));
			as_schedule_single_action($start_ts + $duration, 'wpam_auction_end', array(
				$auction_id
			));
		}
	...
	public function schedule_cron()
	{
		...
		foreach ($query->posts as $post)
		{
			...
			if ($start && !as_has_scheduled_action('wpam_auction_start', array(
				$post->ID
			)))
			{
				as_schedule_single_action($start, 'wpam_auction_start', array(
					$post->ID
				));
			}
			...
			if ($end && !as_has_scheduled_action('wpam_auction_end', array(
				$post->ID
			)))
			{
				as_schedule_single_action($end, 'wpam_auction_end', array(
					$post->ID
				));
			}
		}
	...
	protected function determine_state($auction_id)
	{
		...
		{
			add_action('admin_notices', function () use ($auction_id)
			{
				echo '<div class="notice notice-warning"><p>' . sprintf(esc_html__('Auction #%d is missing or has invalid start/end dates and has been set to scheduled.', 'wpam') , $auction_id) . '</p></div>';
			});
			update_post_meta($auction_id, '_auction_status', 'scheduled');
	...
	public static function parse_increment_rules($rules)
	{
		...
		foreach ($lines as $line)
		{
			...
			list($max, $inc) = array_map('trim', explode('|', $line) + array(
				null,
				null
			));
	...
	public function cli_auction_status($args, $assoc_args)
	{
		...
		\WP_CLI\Utils\format_items('table', $items, array(
			'ID',
			'title',
			'status'
		));

...
class WPAM_Bid {
	...
	public function __construct() {
		add_action( 'wp_ajax_wpam_place_bid', array( $this, 'place_bid' ) );
		add_action( 'wp_ajax_nopriv_wpam_place_bid', array( $this, 'place_bid' ) );

		add_action( 'wp_ajax_wpam_buy_now', array( $this, 'buy_now' ) );
		add_action( 'wp_ajax_nopriv_wpam_buy_now', array( $this, 'buy_now' ) );

		add_filter( 'woocommerce_account_menu_items', array( $this, 'add_account_menu_items' ) );
		add_action( 'woocommerce_account_my-bids_endpoint', array( $this, 'render_my_bids_page' ) );
		add_action( 'woocommerce_account_auctions-won_endpoint', array( $this, 'render_auctions_won_page' ) );

		add_action( 'wp_ajax_wpam_get_highest_bid', array( $this, 'get_highest_bid' ) );
		add_action( 'wp_ajax_nopriv_wpam_get_highest_bid', array( $this, 'get_highest_bid' ) );
	...
	public function place_bid() {
		...
		if ( ! current_user_can( 'auction_bidder' ) ) {
			wp_send_json_error( array( 'message' => __( 'You are not allowed to bid', 'wpam' ) ) );
		}

		if ( get_option( 'wpam_require_kyc' ) && ! get_user_meta( $user_id, 'wpam_kyc_verified', true ) ) {
			wp_send_json_error( array( 'message' => __( 'Verification required', 'wpam' ) ) );
		}
		...
		if ( $now < $start_ts || $now > $end_ts ) {
			wp_send_json_error( array( 'message' => __( 'Auction not active', 'wpam' ) ) );
		}
		...
		if ( $max_bids > 0 ) {
			...
			if ( $count >= $max_bids ) {
				wp_send_json_error( array( 'message' => __( 'Bid limit reached', 'wpam' ) ) );
			}
		}
		...
		if ( $sealed ) {
			...
			if ( $placed > 0 ) {
				wp_send_json_error( array( 'message' => __( 'Only one bid allowed for sealed auctions', 'wpam' ) ) );
			}
		}
	...
	public function buy_now() {
		...
		if ( ! $enabled || ! $price ) {
			wp_send_json_error( array( 'message' => __( 'Buy now unavailable', 'wpam' ) ) );
		}
	...
	public function render_my_bids_page() {
		$user_id = get_current_user_id();
		$bids    = $user_id ? $this->get_user_bids( $user_id ) : array();
		include WPAM_PLUGIN_DIR . 'templates/my-bids.php';
	}

	public function render_auctions_won_page() {
		$user_id  = get_current_user_id();
		$auctions = $user_id ? $this->get_user_won_auctions( $user_id ) : array();
